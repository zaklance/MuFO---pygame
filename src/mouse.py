...
class Laser(pygame.sprite.Sprite):
    def __init__(self, x, y, target_x, target_y, speed, max_range, y_offset=0):
        super().__init__()
        self.original_image = pygame.Surface((5, 20))
        self.original_image.fill('red')
        self.image = self.original_image.copy()
        self.rect = self.image.get_rect(center=(x, y + y_offset))
    ...
    def handle_click(self, player):
        ...
        if pygame.mouse.get_pressed()[0]:  # Left click
            if current_time - self.last_shot_time > self.shoot_cooldown:
                ...
                laser = Laser(player_center[0], player_center[1], mouse_pos[0], mouse_pos[1], speed=10, max_range=player.rect.width * 5, y_offset=-50)
